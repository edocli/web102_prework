...
function filterUnfundedOnly() {
    ...
    const newGames = GAMES_JSON.filter(game => game.pledged < game.goal);
    console.log(newGames.length)

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(newGames);
}
...
const displayStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${GAMES_JSON.length} games. Currently, ${count} ${count === 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these amazing games!`;
